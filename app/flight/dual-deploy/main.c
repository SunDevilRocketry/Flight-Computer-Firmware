...
if ( header_status != DATA_LOG_OK )
	{
	error_fail_fast( ERROR_DATA_LOG_LOAD_HEADER_ERROR );
	}
...
if ( header_status != DATA_LOG_OK )
	...
	if ( header_status != DATA_LOG_OK )
		{
		error_fail_fast( ERROR_DATA_LOG_CORRECT_HEADER_ERROR );
		}
	}
...
if ( data_log_status != DATA_LOG_OK )
	{
	error_fail_fast( ERROR_DATA_LOG_PREP_MEM_ERROR );
	}
...
if ( press_fifo_status != PRESS_FIFO_OK )
	{
	error_fail_fast( ERROR_CAL_GROUND_ALT_ERROR );
	}
...
while ( usb_detect() )
	...
	if ( ( usb_status == USB_OK ) && ( command != 0 ) )
		...
		if ( terminal_status != TERMINAL_OK )
			{
			error_fail_fast( ERROR_TERMINAL_ERROR );
			}
